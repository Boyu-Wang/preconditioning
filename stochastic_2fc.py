...
def main():
    ...
    t = time.time()
    save_name_prefix = 'minibatch_2fc_pre-%s_k-%d_decay-%.5f_lr-%.4f_batchsize-%d'%(args.precondition, args.k, args.weight_decay, args.lr, args.batch_size)
    logger = Logger(os.path.join('result', '%s_log.txt'%(save_name_prefix)))

    loss_tr_all = []
    loss_tst_all = []
    for epoch in range(args.epochs):
        loss_tr = run_epoch(trLD, model, criterion, len(trSet), is_train=True, optimizer=optimizer)
        loss_tst = run_epoch(tstLD, model, criterion, len(tstSet), is_train=False, optimizer=None)
